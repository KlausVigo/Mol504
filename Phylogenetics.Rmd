...
Benutzen Sie die Datei *concatenated_dna.fasta* um einen phylogenetischen Baum zu  berechenen (http://iqtree.cibiv.univie.ac.at/). 
Benutzen Sie die Standardeinstellungen.

* Welches Model wurde verwendet?

* Sind die unterscheiedlichen Familien (Katzen, Hunde, Bären, etc. monophyletic?
...
Öffnen Sie RStudio und kopieren Sie den folgenden Code in die Konsole (Console).
...
Danach laden wir 3 Datensätze, einen phylogenitischen Baum, ein Alignment und eine Tabelle mit information zu unseren Daten.  
```{R, eval=TRUE}
...
help(plot.phylo)
```
Wir können die Species von dem Baum löschen, da es scheinbar mit der Sequenz Probleme gibt. 
```{R, eval=FALSE}
